...
class NetworkManager extends GetxController {
 ...
 void onInit() {
   super.onInit();
   // Listen to connectivity changes and call _updateConnectionStatus without casting
   _connectivitySubscription = _connectivity.onConnectivityChanged.listen((event) => _updateConnectionStatus(event.first));
 }
 
 /// Update the connection status on changes in connectivity and show a relevant popup for no internet connection.
 Future<void> _updateConnectionStatus(ConnectivityResult result) async {
   _connectionStatus.value = result;
 
   if (_connectionStatus.value == ConnectivityResult.none) {
     // Show no internet connection warning
     BLoaders.warningSnackBar(title: 'No Internet Connection');
   }
 }
  /// Get the current connection status.
